"""
ابولی لند — Single File Version v2
"""
...
def init_db():
    import pathlib; pathlib.Path(DB_PATH).parent.mkdir(parents=True, exist_ok=True)
...
def use_item_in_duel(did,uid,iid):
    ...
    # فقط معجون و اسپل مصرفی کم میشه — سلاح دائمیه
    _cons_cats=("potions",)
    _cons_spells={"magic_shield","confusion_mist","gods_wrath","magic_tsunami","great_heal","death_star_spell","hell_gate","fireball","ice_arrow","lightning"}
    if cat in _cons_cats or (cat=="spells" and iid in _cons_spells):
        remove_item(uid,iid)
...
async def shop_start(update:Update,ctx:ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"🏪 *فروشگاه ابولی لند*\n\n👤 {u.first_name}\n💎 امتیاز: *{ud['points']}*\n\nیه دسته انتخاب کن:",
...
async def cb_sb(update:Update,ctx:ContextTypes.DEFAULT_TYPE):
    ...
    await q.edit_message_text(f"🏪 *فروشگاه ابولی لند*\n\n💎 امتیاز: *{u['points']}*",
        parse_mode="Markdown",reply_markup=InlineKeyboardMarkup(_shop_kb()))
...
async def start_cmd(update:Update,ctx:ContextTypes.DEFAULT_TYPE):
    ...
    if update.effective_chat.type=="private":
        await update.message.reply_text(
            f"⚔️ *سلام {u.first_name}! به ابولی لند خوش اومدید!* 🏰\n\n"
        ...
        await update.message.reply_text(f"⚔️ {u.first_name} به ابولی لند پیوست 🏰!\nبرای خرید آیتم به پیوی بات بیا: @{ctx.bot.username}")
...
def main():
    ...
    logger.info("🤖 ابولی لند Bot v2 starting...")
